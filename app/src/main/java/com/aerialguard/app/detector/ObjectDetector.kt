 ...
 * Runs Google's free, pretrained COCO SSD-MobileNet TFLite model
  * (downloaded automatically by download_model.gradle into
   * app/src/main/assets/detect.tflite + labelmap.txt).
    *
     * That model already recognizes "person", "car", "truck", "bus",
      * "motorcycle", "bicycle", "train", "boat" and "airplane" among its 90
       * COCO classes — this wrapper maps those into the three threat buckets the
        * app cares about: HUMAN, VEHICLE, AIRCRAFT. (Quadcopters are handled
         * separately by [QuadcopterHeuristicDetector] since no free pretrained
          * "quadcopter" model exists — see the README.)
           */
class ObjectDetector(context: Context) {
      ...
      private val vehicleLabels = setOf("car", "truck", "bus", "motorcycle", "bicycle", "train", "boat")
          private val aircraftLabels = setOf("airplane")
                                                                                                    ...
                                                                                                    val count = numDetections[0].toInt().coerceIn(0, 10)
                                                                                                            ...
                                                                                                            for (i in 0 until count) {
                                                                                                                          ...
                                                                                                                          val category = when {
                                                                                                                                            ...
                                                                                                                                            aircraftLabels.contains(label) -> ThreatCategory.AIRCRAFT
