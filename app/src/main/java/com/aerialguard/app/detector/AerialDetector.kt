...
/**
 * Generic YOLOv8 detector, used for both the drone-viewpoint (VisDrone) model
  * and the military-vehicle model -- they share an architecture and differ only
   * in weights and label set.
    *
     * The Task Library cannot run these: it expects SSD-style postprocessed
      * outputs, whereas YOLOv8 emits one raw tensor needing decode plus NMS. So
       * this drives the bare Interpreter and decodes by hand.
        *
         * If the model file is absent the detector reports itself unavailable and
          * returns nothing, and the app carries on with whatever models did load.
           *
            * Tensor shapes are read at runtime and the box coordinate convention is
             * sniffed, because both differ between Ultralytics export versions.
              */
class AerialDetector(
     context: Context,
     private val modelFile: String = VISDRONE_MODEL,
     private val labels: Array<String> = VISDRONE_LABELS,
     override val source: DetectorSource = DetectorSource.AERIAL
 ) : Detector {

     companion object {
              const val VISDRONE_MODEL = "aerial.tflite"
              const val MILITARY_MODEL = "military.tflite"
              ...
              val VISDRONE_LABELS = arrayOf(
                           "pedestrian", "people", "bicycle", "car", "van",
                           "truck", "tricycle", "awning-tricycle", "bus", "motor"
                       )

                      /**
                               * Military class order. Large equipment only -- these are the classes
